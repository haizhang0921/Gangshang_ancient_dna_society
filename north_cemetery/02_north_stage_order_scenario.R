...
#
# THREE-SCENARIO INPUT VERSION — GLOBAL STAGE-ORDER SENSITIVITY MODEL (2026-09-20)
# -----------------------
...
out_dir <- if (length(args) >= 3L) args[[3L]] else "north_R_stage_order"
...
# Sensitivity model: force the complete archaeological Stage order.
# This is deliberately NOT read from the R environment, so a lingering
# ENFORCE_GLOBAL_STAGE_ORDER=FALSE from a previous primary run cannot override it.
...
if (!isTRUE(enforce_global_stage_order)) {
  stop("Internal error: this sensitivity script requires global Stage order = TRUE.")
}
...
metadata <- data.frame(
  ...
  value = c(
    ...
    "Revised 2026-09-11 relations + forced global Stage I<II<III order",
